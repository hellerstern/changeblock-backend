...
def plotly_contribution_plot(contrib_df, target="",
    ...
    contrib_df['base'] = np.round(
        multiplier * contrib_df['base'].astype(float), round)
    ...
    fill_colors = [fill_color_up if y >
                   0 else fill_color_down for y in contribs]
    line_colors = [line_color_up if y >
                   0 else line_color_down for y in contribs]
    ...
    if orientation == 'vertical':
        fig.update_yaxes(title_text='Predicted ' +
                         ('%' if model_output == "probability" else units))
    else:
...
def plotly_lift_curve(lift_curve_df, cutoff=None, percentage=False, add_wizard=True, round=2):
    ...
    if add_wizard:
        if percentage:
            trace2 = go.Scatter(
                ...
                y=[0.0, 100, 100],
                text=[
                    "0%, 0%", f"{lift_curve_df.random_precision[0]:.2f}%, 100%", "100, 100%"],
                name='perfect',
                hoverinfo="text",
            )
        else:
            trace2 = go.Scatter(
                x=[0.0, lift_curve_df['positives'].max(
                ), lift_curve_df['index'].max()],
                y=[0.0, lift_curve_df['positives'].max(
    ...
    if cutoff is not None:
        ...
        cutoff_idx = max(
            0, len(lift_curve_df[lift_curve_df.pred_proba >= cutoff])-1)
        if percentage:
        ...
        cutoff_lift = np.round(
            lift_curve_df['positives'].iloc[cutoff_idx] / lift_curve_df.random_pos.iloc[cutoff_idx], 1)
        cutoff_precision = np.round(
            lift_curve_df['precision'].iloc[cutoff_idx], 2)
...
def plotly_cumulative_precision_plot(lift_curve_df, labels=None, percentile=None,
    ...
    fig = fig.add_trace(go.Scatter(x=lift_curve_df.index_percentage,
                                   y=lift_curve_df['precision_' +
                                                   str(pos_label)].values,
                                   fill='tozeroy',
    ...
    for y_label in range(pos_label, lift_curve_df.y.max()+1):
        ...
        if y_label != pos_label:
            cumulative_y = cumulative_y + \
                lift_curve_df['precision_' + str(y_label)].values
    ...
    fig.update_layout(title=dict(text='Cumulative percentage per category<br>when sampling top X%',
                      ...
                      xaxis=dict(title='Top X% model scores',
                                 spikemode="across", range=[0, 100]),
                      hovermode="x",
                      plot_bgcolor='#fff')
...
def plotly_dependence_plot(X_col, shap_values, interact_col=None,
    ...
    if interact_col is not None and not is_numeric_dtype(interact_col):
        for onehot_col in interact_col.unique().tolist():
            data.append(
                go.Scattergl(
                    x=X_col[interact_col == onehot_col].replace(
                        {na_fill: np.nan}),
    ...
    layout = go.Layout(
        ...
        yaxis=dict(
            title=f"SHAP value ({units})" if units != "" else "SHAP value")
    )
...
def plotly_shap_violin_plot(X_col, shap_values, X_color_col=None, points=False,
    ...
    if points or X_color_col is not None:
        fig = make_subplots(rows=1, cols=2*n_cats,
                            column_widths=[3, 1]*n_cats, shared_yaxes=True)
        showscale = True
    else:
        fig = make_subplots(rows=1, cols=n_cats, shared_yaxes=True)

    shap_range = shap_values.max() - shap_values.min()
    fig.update_yaxes(range=[shap_values.min()-0.1 *
                     shap_range, shap_values.max()+0.1*shap_range])
...
def plotly_pdp(pdp_df,
    ...
    if absolute:
        ...
        pdp_mean = pdp_df.mean().round(round).values - \
            pdp_df.mean().round(round).values[0]
    ...
    if display_index is not None:
        trace1 = go.Scatter(
            ...
            pdp_df.iloc[[display_index]].round(
                round).values[0] - pdp_df.iloc[[display_index]].values[0, 0],
            mode='lines+markers',
...
def plotly_importances_plot(importance_df, descriptions=None, round=3,
    ...
    data = [go.Bar(
        ...
        # don't know why, but order needs to be reversed
        text=descriptions[::-1] if descriptions is not None else None,
        # textposition='inside',
...
def plotly_pr_auc_curve(precision, recall, thresholds, score, cutoff=None, round=2):
    ...
    layout = go.Layout(title='PR AUC CURVE',
                       #    width=450,
                       #    height=450,
                       xaxis=dict(title='Precision', range=[
                                  0, 1], constrain="domain"),
...
def plotly_shap_scatter_plot(X, shap_values_df, display_columns=None, title="Shap values",
    ...
    assert matching_cols(
        X, shap_values_df), "X and shap_values_df should have matching columns!"
    ...
    if highlight_index is not None:
        if isinstance(highlight_index, int):
            ...
            assert str(
                highlight_index) in idxs, f"{highlight_index} not found in idxs!"
...
def plotly_predicted_vs_actual(y, preds, target="", units="", round=2,
    ...
    layout = go.Layout(
        ...
        yaxis=dict(title=f"Predicted {target}" +
                   (f" ({units})" if units else "")),
        xaxis=dict(title=f"Observed {target}" +
                   (f" ({units})" if units else "")),
        plot_bgcolor='#fff',
        hovermode='closest',
    )
...
def plotly_residuals_vs_col(y, preds, col, col_name=None, residuals='difference',
    ...
    if not is_numeric_dtype(col):
        ...
        if points:
            for i in range(n_cats):
                fig.update_xaxes(showgrid=False, zeroline=False,
                                 visible=False, row=1, col=2+i*2)
                fig.update_yaxes(
                    showgrid=False, zeroline=False, row=1, col=2+i*2)
        ...
        trace1 = go.Scattergl(
            x=col,
            y=np.ones(len(preds)) if residuals == 'ratio' else np.zeros(
                len(preds)),
            mode='lines',
            name=col_name,
            hoverinfo="none",
    ...
    if safe_isinstance(model, "RandomForestClassifier", "ExtraTreesClassifier"):
        preds_df = (
            pd.DataFrame({
                ...
                'prediction': [
                    np.round(100*m.predict_proba(observation)
                             [0, pos_label], round)
...
def plotly_xgboost_trees(xgboost_preds_df, highlight_tree=None, y=None, round=2,
    ...
    if is_classifier:
        ...
        texts = [f"tree no {t}:<br>change = {d:.{round}f}<br> click for detailed info"
                 for (t, d) in zip(trees, diffs)]
        texts.insert(
            0, f"Base prediction: <br>pred = {base_prediction:.{round}f}")
        texts.append(
            f"Final Prediction: <br>pred = {final_prediction:.{round}f}")
    ...
    fill_colors = [fill_color_up if diff >
                   0 else fill_color_down for diff in diffs]
    line_colors = [line_color_up if diff >
                   0 else line_color_down for diff in diffs]
